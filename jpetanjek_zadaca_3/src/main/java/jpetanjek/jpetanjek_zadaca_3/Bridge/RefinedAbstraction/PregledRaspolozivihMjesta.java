 ...
 */
public class PregledRaspolozivihMjesta extends Aktivnost implements IAktivnost {
                    ...
                    if (Tvrtka.getInstanca().getVoziloById(id_vrste_vozila) != null) {
                        ...
                        if (kapacitet != null) {
                            ...
                            Tvrtka.getInstanca().getController().setModel("U " + vrijeme + " korisnik " + osoba.getImePrezime()
                                                + " traži na lokaciji " + lokacija.getNaziv() + " broj ("+(kapacitet.getBroj_mjesta()-kapacitet.getVozila().size())+") "
                                                + "raspoloživih mjesta za "+ Tvrtka.getInstanca().getVoziloById(id_vrste_vozila).getNaziv() +"."
                                        );
